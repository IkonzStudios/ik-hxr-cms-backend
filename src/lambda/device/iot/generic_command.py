...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Check if body has command
        if not body:
            return create_error_response(400, DEVICE_ERROR_MESSAGES["EMPTY_REQUEST_BODY"])

        command = body.get("command")
        if not command:
            return create_error_response(400, "Command is required")

        description = body.get("description", "")  # Optional field

        # Validate command
        if not isinstance(command, str) or not command.strip():
            return create_error_response(400, DEVICE_ERROR_MESSAGES["INVALID_COMMAND"])
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                command_result = {
                    "success": True,
                    "message": DEVICE_SUCCESS_MESSAGES["COMMAND_SENT"],
                    "command": command,
                    "description": description,
                    "iot_response": iot_response
                }
                
                print(f"Command sent successfully to device {device_id}")
                return create_device_response(existing_device, {"command_execution": command_result})
            else:
