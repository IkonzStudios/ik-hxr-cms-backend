...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"User {user_info['user_id']} ({user_info['role']}) configuring brightness for device {device_id}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Create brightness configuration result
                brightness_config_result = {
                    "success": True,
                    "message": DEVICE_SUCCESS_MESSAGES["BRIGHTNESS_CONFIGURED"],
                    "brightness": brightness,
                    "iot_response": iot_response
                }
                
                print(f"Brightness configuration successful for device {device_id}")
                return create_device_response(existing_device, {"brightness_configuration": brightness_config_result})
            else:
