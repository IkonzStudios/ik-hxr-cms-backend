...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # RBAC: Check if user has permission to create playlists in this organization
        rbac_error, user_info = check_create_permission_with_org(event, "playlist", organization_id)
        if rbac_error:
            return rbac_error

        # Log user action for audit
        print(f"User {user_info['user_id']} ({user_info['role']}) creating playlist in org {organization_id}")
