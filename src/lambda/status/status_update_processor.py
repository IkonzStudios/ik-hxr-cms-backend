...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    """
    Lambda function to process queued IoT device status updates sequentially.
    
    This function is triggered by SQS messages and processes them one by one
    to avoid race conditions when multiple IoT devices send status updates concurrently.
    
    Expected SQS event structure:
    {
        "Records": [
            {
                "body": "{\"timestamp\": \"2025-09-05T09:52:32.050217+00:00\", \"thingName\": \"device-id\", \"jobId\": \"job-id\", \"step\": \"DOWNLOAD_1_SUCCESS\", \"message\": \"Success message\"}"
            }
        ]
    }
    """
    ...
    try:
        ...
        for record in event.get("Records", []):
            try:
                ...
                device_id = message_body["thingName"]  # thingName is the device_id
                content_key = message_body.get("key", "None")
                job_id = message_body["jobId"]
                step = message_body["step"]
                ...
                result = route_status_update(device_id, content_key, job_id, step, message, timestamp, message_body)
...
def handle_assign_content(device_id: str, content_id: str, job_id: str, step: str, message: str, timestamp: str, full_body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if step.endswith("_STARTED"):
        ...
        # For skipped downloads, content was already available on device
        # Move from initiated/downloading to downloaded
        print(f"Content {content_id} download was skipped (already available)")
        if content_id in contents_initiated:
            contents_initiated.remove(content_id)
            print(f"Removed {content_id} from initiated array")
        if content_id in contents_downloading:
            contents_downloading.remove(content_id)
            print(f"Removed {content_id} from downloading array")
        if content_id in contents_failed:
            contents_failed.remove(content_id)
            print(f"Removed {content_id} from failed array")
        if content_id not in contents_downloaded:
            contents_downloaded.append(content_id)
            print(f"Added {content_id} to downloaded array")
